...
## Entregável 1 — SKILL.md: `create-integration-test`
...
import { mockAzureSearchResponse, mockAzureOpenAICompletion } from '../mocks/handlers'
import { chunks } from '../fixtures/chunks'
import { queries } from '../fixtures/queries'
...
describe('QueryHandler', () => {
  describe('dangerous goods guardrail', () => {
    it('should return explicit rejection when query involves dangerous goods return', async () => {
      ...
      const body = JSON.parse(response.body)
      expect(body.answer).toMatch(/não.*elegível|processo especial|Gestão de Riscos/i)
  ...
  describe('no-match fallback', () => {
    it('should return fallback message and null source_document when no chunk matches', async () => {
      ...
      server.use(
        ...
        mockAzureOpenAICompletion('Não encontrei informação sobre isso na documentação disponível.')
      )
...
## Entregável 2 — Checklist de Revisão de Testes (verificável em < 2 minutos)

```markdown
# Checklist de Revisão de Testes — NovaTech Assistant
